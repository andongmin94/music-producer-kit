...
class OfflineDistributionTests(unittest.TestCase):
    ...
    def test_every_module_and_reference_has_a_local_file(self):
        # All retained files are accessible; there is no alternate view.
    ...
    def test_selected_source_counts(self):
        result = package.check(self.root)['local_source_library']
        self.assertEqual((result['sources'], result['modules'], result['files']), (2, 41, 89))
        self.assertEqual(result['bytes'], 1805308)
